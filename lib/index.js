...
Astronomer.prototype.track = function(track) {
  ...
  var params = {
    Data: record,
    StreamName: self.config.streamName,
  ...
  if (expireTime && expireTime.getTime() <= Date.now()) {
    self.auth(function(err, response) {
      console.log(self.kinesis.config.credentials);
      self.kinesis.config.credentials.expired = true;
      self.kinesis.config.credentials.WebIdentityToken = response.body.credentials.Token;
      self.putRecord(params);
    });
  } else {
    self.putRecord(params);
  }
...
Astronomer.prototype.putRecord = function(params) {
