...
Astronomer.prototype.ensureKinesisConfig = function(callback) {
  ...
  self.requestConfig(function(err, response) {
    ...
    // In production this will be null, and endpoint will automatcically
    // be assigned according to region.  In dev, we specify localhost.
    if (self.options.kinesisServer) {
      params.endpoint = self.options.kinesisServer;
    }
