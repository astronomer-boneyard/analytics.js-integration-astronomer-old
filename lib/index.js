...
var Astronomer = module.exports = exports = integration('astronomer')
  ...
  .option('credentialsExpiration', 3600)
...
Astronomer.prototype.initialize = function() {
  ...
  // Refresh credentials a little before they expire
  setInterval(function() {
    self.auth(function(err, response) {
      self.configAws(response.body);
    });
  }, self.options.credentialsExpiration * 1000 * 0.75);
};
 ...
 * @param {Function} callback Callback
 */
Astronomer.prototype.auth = async.retry(3, function(callback) {
