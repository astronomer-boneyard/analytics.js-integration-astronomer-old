...
Astronomer.prototype.expired = function() {
  ...
  return ((!expireTime || expireTime.getTime() <= Date.now()) && !self.authing);
};
...
Astronomer.prototype.ensureKinesisConfig = function(callback) {
  ...
  if (!self.expired()) {
    return callback(self.config, self.kinesis);
  }

  // Request fresh config (token)
  self.requestCredentials(function(err, response) {
    console.log('authed', err, response);
...
Astronomer.prototype.requestCredentials = function(callback) {
